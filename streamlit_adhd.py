...
def load_data():
    """Charge le dataset TDAH depuis Google Drive avec gestion d'erreur robuste"""
    try:
        ...
        column_mapping = {
            'diagnosis': 'TDAH', 'target': 'TDAH', 'label': 'TDAH',
            'age': 'Age', 'AGE': 'Age',
            'gender': 'Genre', 'sex': 'Genre', 'Sex': 'Genre',
            'inattention': 'Inattention_Score',
            'hyperactivity': 'Hyperactivite_Score',
            'impulsivity': 'Impulsivite_Score'
        }
        ...
        # Conversion de la variable TDAH
        if 'TDAH' in df.columns:
            # Essayer différents mappings
            unique_values = df['TDAH'].unique()
            if set(unique_values).issubset({0, 1, '0', '1'}):
                df['TDAH'] = df['TDAH'].astype(str).map({'1': 'Oui', '0': 'Non'})
            elif set(unique_values).issubset({'Yes', 'No', 'YES', 'NO'}):
                df['TDAH'] = df['TDAH'].str.lower().map({'yes': 'Oui', 'no': 'Non'})
...
def advanced_preprocessing(df, target_column='TDAH'):
    """Préprocessing avancé avec feature engineering"""
    ...
    # Création de nouvelles features si les colonnes scores existent
    ...
    # Binning de l'âge si disponible
...
# =================== FONCTIONS MACHINE LEARNING AVANCÉES ===================

@st.cache_resource
def train_multiple_models(df, target_column='TDAH'):
    """Entraîne plusieurs modèles ML avec optimisation d'hyperparamètres"""
    try:
        ...
        if len(numeric_features) == 0:
            st.error("❌ Aucune feature numérique trouvée")
            return None, None, None, None
...
def perform_feature_analysis(df, target_column='TDAH'):
    """Analyse des features avec sélection automatique"""
    if df is None or target_column not in df.columns:
        return None
...
def page_accueil():
    ...
    <p>Cet outil utilise l'intelligence artificielle pour le dépistage du TDAH à des fins de recherche et d'information uniquement.
    ...
    if df is not None:
        with col1:
            ...
            <p>Échantillons dans les données</p>
            </div>
            """, unsafe_allow_html=True)
    ...
    with col1:
        ...
        est un trouble neurodéveloppemental caractérisé par trois symptômes principaux :</p>

        <h4 style="color: #1976d2;">🎯 Inattention</h4>
        <ul>
        <li>Difficultés de concentration et d'attention soutenue</li>
        <li>Erreurs d'inattention dans les tâches</li>
        <li>Problèmes d'organisation et de planification</li>
        <li>Évitement des tâches nécessitant un effort mental</li>
        </ul>

        <h4 style="color: #1976d2;">⚡ Hyperactivité</h4>
        <ul>
        <li>Agitation motrice excessive</li>
        <li>Difficulté à rester assis ou calme</li>
        <li>Sensation d'être constamment "en mouvement"</li>
        ...
        <li>Interruption fréquente des autres</li>
        <li>Prises de décision rapides sans réflexion</li>
        <li>Difficultés de self-contrôle</li>
        </ul>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col2:
        ...
        fig = px.pie(
            ...
            title="Répartition des symptômes TDAH",
...
def page_exploration():
    """Page d'exploration des données approfondie"""
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["📈 Vue d'ensemble", "🔍 Analyse par variable", "🔗 Corrélations", "🎯 Feature Engineering"])
        ...
        if 'TDAH' in df_processed.columns:
            ...
            with col1:
                ...
                fig = px.pie(values=tdah_counts.values, names=tdah_counts.index,
                           title="Répartition TDAH vs Non-TDAH",
    ...
    with tab2:
        ...
        selected_var = st.selectbox("Choisir une variable à analyser", df_processed.columns)
    ...
    with tab3:
        ...
        if len(numeric_df.columns) > 1:
            corr_method = st.selectbox("Méthode de corrélation", ["pearson", "spearman", "kendall"])
...
def page_machine_learning():
    """Page de machine learning avancé avec optimisation"""
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["📊 Comparaison des modèles", "🎯 Performance détaillée", "📈 Courbes ROC", "⚙️ Hyperparamètres"])
    ...
    with tab1:
        ...
        with col1:
            ...
            fig = px.bar(performance_df, x='Modèle', y=['Accuracy', 'AUC-ROC', 'CV Score'],
        ...
        <p>Ce modèle montre les meilleures performances pour la classification TDAH.</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with tab2:
        ...
        selected_model = st.selectbox("Sélectionner un modèle pour l'analyse détaillée",
                                     list(results.keys()))
        ...
        if selected_model in results:
            ...
            with col2:
                ...
                fig = px.histogram(prob_df, x='Probabilité', color='Classe réelle',
    ...
    with tab3:
        ...
        # Analyse des seuils
        st.subheader("⚖️ Analyse des seuils de classification")

        selected_model_roc = st.selectbox("Sélectionner un modèle pour l'analyse des seuils",
        ...
        if selected_model_roc in results:
            ...
            fig = px.line(threshold_df, x='Seuil', y=['Accuracy', 'Sensibilité', 'Spécificité'],
                         title=f"Métriques par seuil - {selected_model_roc}")
            st.plotly_chart(fig, use_container_width=True)

    with tab4:
        # Hyperparamètres optimaux
        st.subheader("⚙️ Hyperparamètres optimisés")

        for name, model_results in results.items():
            with st.expander(f"📋 {name} - Paramètres optimaux"):
    ...
    st.subheader("💾 Sauvegarde du meilleur modèle")
    ...
    if st.button("Sauvegarder le meilleur modèle", type="primary"):
        try:
            ...
            st.success(f"✅ Modèle {best_model_name} sauvegardé avec succès!")
            st.balloons()
...
def page_prediction():
    ...
    la probabilité de TDAH basée sur vos réponses. Les résultats sont à des fins
    d'information uniquement et ne remplacent pas un diagnostic médical.</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    try:
        ...
        st.success(f"✅ Modèle {model_data['model_name']} chargé")
        ...
        st.warning("⚠️ Aucun modèle sauvegardé trouvé. Entraînez d'abord un modèle dans la section Machine Learning.")

        # Entraînement temporaire pour la démonstration
        df = load_data()
        if df is not None:
            df_processed, _ = advanced_preprocessing(df)
            if df_processed is not None and 'TDAH' in df_processed.columns:
                with st.spinner("Entraînement d'un modèle temporaire..."):
                    ...
                    if results is not None:
                        ...
                        st.success("✅ Modèle temporaire créé")
                    else:
                        st.error("❌ Impossible de créer un modèle")
                        return
            else:
    ...
    st.subheader("📝 Saisie des informations pour la prédiction")
    ...
    with st.form("prediction_form"):
        ...
        with col1:
            age = st.number_input("Âge", min_value=6, max_value=80, value=25)
            genre = st.selectbox("Genre", ["Féminin", "Masculin"])
        ...
        with col2:
            ...
            situation_pro = st.selectbox("Situation professionnelle",
                                       ["Étudiant", "Employé", "Indépendant", "Sans emploi", "Retraité"])

        # Scores comportementaux
        st.markdown("**Scores comportementaux (1-10, 10 = symptômes très présents)**")
        col1, col2, col3 = st.columns(3)

        with col1:
            inattention = st.slider("Score d'inattention", 1.0, 10.0, 5.0, 0.5,
                                  help="Difficultés de concentration, erreurs d'inattention, oublis")

        with col2:
            hyperactivite = st.slider("Score d'hyperactivité", 1.0, 10.0, 5.0, 0.5,
                                    help="Agitation motrice, difficulté à rester assis")

        with col3:
            impulsivite = st.slider("Score d'impulsivité", 1.0, 10.0, 5.0, 0.5,
                                  help="Interruptions, difficultés à attendre, décisions impulsives")
        ...
        with col1:
            sommeil = st.slider("Problèmes de sommeil", 1.0, 10.0, 5.0, 0.5)
            anxiete = st.slider("Niveau d'anxiété", 1.0, 10.0, 5.0, 0.5)

        with col2:
            stress = st.slider("Niveau de stress", 1.0, 10.0, 5.0, 0.5)
            concentration = st.slider("Difficultés de concentration", 1.0, 10.0, 5.0, 0.5)
        ...
        with col1:
            antecedents_familiaux = st.selectbox("Antécédents familiaux TDAH", ["Non", "Oui", "Incertain"])
            troubles_apprentissage = st.selectbox("Troubles d'apprentissage", ["Non", "Oui", "Incertain"])

        with col2:
            medicaments = st.selectbox("Prise de médicaments psychotropes", ["Non", "Oui"])
            suivi_psy = st.selectbox("Suivi psychologique actuel", ["Non", "Oui"])

        predict_button = st.form_submit_button("🔮 Effectuer la prédiction", type="primary")
    ...
    if predict_button:
        try:
            ...
            # Création du vecteur de features (adapté aux features du modèle)
            input_features = [
            ...
            # Compléter ou tronquer selon les besoins
            while len(input_features) < expected_features:
                input_features.append(5.0)  # Valeur par défaut
            ...
            with col1:
                st.metric(
                    ...
                    delta=f"{'Élevé' if risk_percentage > 60 else 'Modéré' if risk_percentage > 40 else 'Faible'}"
                )

            with col2:
                prediction_text = "TDAH Probable" if prediction == 1 else "TDAH Peu Probable"
                confidence = max(prediction_proba) * 100
                st.metric("Prédiction", prediction_text, f"Confiance: {confidence:.1f}%")

            with col3:
                st.metric("Modèle utilisé", model_data['model_name'],
            ...
            fig = go.Figure(go.Indicator(
                ...
                domain={'x': [0, 1], 'y': [0, 1]},
                title={'text': "Probabilité de TDAH (%)"},
                delta={'reference': 50},
                gauge={
                    'axis': {'range': [None, 100]},
                    'bar': {'color': "#1976d2"},
            ...
            categories = ['Inattention', 'Hyperactivité', 'Impulsivité', 'Sommeil', 'Anxiété', 'Stress']
            ...
            if risk_percentage > 70:
                st.markdown("""
                <div class="warning-box">
                <h4>🔴 Risque élevé de TDAH détecté</h4>
                <p><strong>Recommandations urgentes :</strong></p>
                <ul>
                <li>📞 Consultez rapidement un professionnel de santé spécialisé (psychiatre, neurologue)</li>
                <li>📋 Demandez une évaluation diagnostique complète</li>
                <li>📝 Préparez un historique détaillé de vos symptômes</li>
                <li>👥 Informez votre entourage proche de vos difficultés</li>
                </ul>
                </div>
                """, unsafe_allow_html=True)
                ...
                st.markdown("""
                <div class="warning-box">
                <h4>🟡 Risque modéré de TDAH</h4>
                <p><strong>Recommandations :</strong></p>
                <ul>
                <li>🩺 Consultez votre médecin traitant pour discuter de vos symptômes</li>
                <li>📊 Surveillez l'évolution de vos difficultés</li>
                <li>📚 Documentez vos symptômes dans un journal</li>
                <li>🧘 Explorez des stratégies de gestion du stress</li>
                </ul>
                </div>
                """, unsafe_allow_html=True)
                ...
                st.markdown("""
                <div class="success-box">
                <h4>🟢 Risque faible de TDAH</h4>
                <p><strong>Informations :</strong></p>
                <ul>
                <li>✅ Vos réponses ne suggèrent pas la présence de TDAH</li>
                <li>👀 Continuez à surveiller vos symptômes si vous avez des préoccupations</li>
                <li>💪 Maintenez de bonnes habitudes de vie</li>
            ...
            risk_factors = []
            if inattention >= 7:
                risk_factors.append("Score d'inattention élevé")
            if hyperactivite >= 7:
                risk_factors.append("Score d'hyperactivité élevé")
            if impulsivite >= 7:
                risk_factors.append("Score d'impulsivité élevé")
            if antecedents_familiaux == "Oui":
                risk_factors.append("Antécédents familiaux présents")
            if sommeil >= 7:
                risk_factors.append("Problèmes de sommeil importants")
            ...
            st.info("💡 Vérifiez que le modèle est correctement entraîné dans la section Machine Learning.")
...
def page_test_asrs():
    ...
    <p>L'Adult ADHD Self-Report Scale (ASRS-v1.1) est l'outil de dépistage de référence
    développé par l'Organisation Mondiale de la Santé en collaboration avec Harvard Medical School.
    Il comprend 18 questions basées sur les critères DSM-5 pour le TDAH chez l'adulte.</p>
    </div>
    """, unsafe_allow_html=True)

    # Questions ASRS-v1.1 complètes (version française officielle)
    questions_part_a = {
    ...
    questions_part_b = {
        ...
        15: "À quelle fréquence vous trouvez-vous en train de trop parler dans des situations sociales ?",
        16: "Quand vous êtes en conversation, à quelle fréquence vous surprenez-vous à finir les phrases des personnes à qui vous parlez, avant qu'elles puissent les finir elles-mêmes ?",
        17: "À quelle fréquence avez-vous des difficultés à attendre votre tour dans des situations où chacun doit attendre son tour ?",
        18: "À quelle fréquence interrompez-vous les autres quand ils sont occupés ?"
    }

    # Options de réponse
    options = ["Jamais", "Rarement", "Parfois", "Souvent", "Très souvent"]

    # Initialisation des réponses dans le session state
    ...
    with st.form("asrs_questionnaire"):
        # Part A - Questions de dépistage
        st.markdown('<h3 style="color: #1976d2;">📋 Partie A - Questions de dépistage principales</h3>', unsafe_allow_html=True)
        st.markdown("*Ces 6 questions sont les plus prédictives du TDAH selon les recherches de l'OMS*")

        for q_num, text in questions_part_a.items():
            st.session_state.asrs_responses[q_num] = st.radio(
                f"**Question {q_num}:** {text}",
                options=options,
                index=0,  # "Jamais" par défaut
                key=f"q{q_num}",
                help="Choisissez la fréquence qui correspond le mieux à votre expérience"
            )

        st.markdown("---")

        # Part B - Questions complémentaires
        st.markdown('<h3 style="color: #1976d2;">📋 Partie B - Questions complémentaires</h3>', unsafe_allow_html=True)
        st.markdown("*Ces questions permettent une évaluation plus complète des symptômes*")
        ...
        submitted = st.form_submit_button("🔍 Calculer mon score ASRS", type="primary")

    if submitted:
        # Vérification que toutes les questions ont une réponse
        if len(st.session_state.asrs_responses) < 18:
            st.error("❌ Veuillez répondre à toutes les questions avant de calculer le score.")
            return

        # Calcul des scores selon les critères officiels ASRS
        score_mapping = {"Jamais": 0, "Rarement": 1, "Parfois": 2, "Souvent": 3, "Très souvent": 4}
        ...
        # Critères de dépistage positif pour Part A (selon recherches OMS)
        # Seuils spécifiques par question pour Part A
        part_a_thresholds = [2, 2, 2, 2, 2, 2]  # Seuils cliniques validés
        part_a_positive = sum([1 for i, score in enumerate(part_a_scores) if score >= part_a_thresholds[i]])

        # Analyse par domaine (Inattention vs Hyperactivité/Impulsivité)
        ...
        # Affichage des résultats
        st.success("✅ Questionnaire ASRS-v1.1 complété!")

        # Métriques principales
        col1, col2, col3, col4 = st.columns(4)

        with col1:
            st.metric("Score Partie A", f"{part_a_total}/24", f"{part_a_positive}/6 critères positifs")
        ...
        with col4:
            risk_level = "Élevé" if part_a_positive >= 4 else "Modéré" if part_a_positive >= 2 else "Faible"
            st.metric("Niveau de risque", risk_level)

        # Interprétation clinique officielle
        st.subheader("🎯 Interprétation clinique")

        if part_a_positive >= 4:
            st.markdown("""
            <div class="warning-box">
            <h4>🔴 Dépistage POSITIF - Symptômes hautement compatibles avec un TDAH</h4>
            <p><strong>Signification clinique :</strong> Vos réponses à la Partie A indiquent une forte probabilité
            de présence de symptômes TDAH selon les critères de l'OMS.</p>

            <p><strong>Recommandations urgentes :</strong></p>
            <ul>
            <li>📞 <strong>Consultez rapidement un professionnel de santé spécialisé</strong> (psychiatre, neurologue, médecin formé au TDAH)</li>
            <li>📋 Demandez une évaluation diagnostique complète incluant entretien clinique et tests neuropsychologiques</li>
            <li>📝 Préparez un historique détaillé de vos symptômes depuis l'enfance</li>
            <li>👥 Contactez des associations de patients TDAH pour support et information</li>
            </ul>

            <p><strong>⚠️ Important :</strong> Ce test de dépistage ne constitue pas un diagnostic.
            Seul un professionnel de santé qualifié peut poser un diagnostic de TDAH.</p>
            </div>
            """, unsafe_allow_html=True)
            ...
            st.markdown("""
            <div class="warning-box">
            <h4>🟡 Dépistage MODÉRÉ - Certains symptômes TDAH présents</h4>
            <p><strong>Signification clinique :</strong> Vos réponses suggèrent la présence de certains symptômes
            compatibles avec le TDAH, nécessitant une attention particulière.</p>

            <p><strong>Recommandations :</strong></p>
            <ul>
            <li>🩺 Consultez votre médecin traitant pour discuter de vos préoccupations</li>
            <li>📊 Surveillez l'évolution de vos symptômes sur plusieurs semaines</li>
            <li>📚 Tenez un journal de vos difficultés quotidiennes</li>
            <li>🧘 Explorez des stratégies de gestion des symptômes (organisation, mindfulness)</li>
            <li>👥 Considérez un suivi spécialisé si les symptômes persistent ou s'aggravent</li>
            </ul>
            </div>
            """, unsafe_allow_html=True)
            ...
            st.markdown("""
            <div class="success-box">
            <h4>🟢 Dépistage NÉGATIF - Peu de symptômes TDAH détectés</h4>
            <p><strong>Signification clinique :</strong> Vos réponses ne suggèrent pas la présence
            de symptômes TDAH significatifs selon les critères de dépistage de l'OMS.</p>

            <p><strong>Informations importantes :</strong></p>
            <ul>
            <li>✅ Vos difficultés actuelles peuvent avoir d'autres causes (stress, fatigue, autres troubles)</li>
            <li>👀 Continuez à surveiller vos symptômes - le TDAH peut se manifester différemment selon les périodes</li>
            <li>💪 Maintenez de bonnes habitudes de vie (sommeil, exercice, organisation)</li>
            <li>🩺 N'hésitez pas à consulter si vous avez d'autres préoccupations de santé mentale</li>
            </ul>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col1:
            domains_df = pd.DataFrame({
                'Domaine': ['Inattention', 'Hyperactivité/Impulsivité'],
            ...
            fig = px.bar(domains_df, x='Domaine', y='Pourcentage',
                        title="Répartition des symptômes par domaine (%)",
                        color='Pourcentage',
        ...
        with col2:
            # Répartition des réponses par fréquence
            ...
            fig = px.pie(values=response_counts.values, names=response_counts.index,
                        title="Répartition de vos réponses par fréquence",
                        color_discrete_sequence=px.colors.qualitative.Set3)
            fig.update_layout(height=400)
            st.plotly_chart(fig, use_container_width=True)

        # Graphique radar détaillé
        st.subheader("🎯 Profil détaillé des symptômes")
        ...
        themes = {
            ...
            'Mémoire': [3],
            'Procrastination': [4],
            'Hyperactivité motrice': [5, 12],
            'Hyperactivité mentale': [6, 13, 14],
            'Impulsivité verbale': [15, 16],
            'Impulsivité comportementale': [17, 18]
        }
        ...
        fig.update_layout(
            polar=dict(
                radialaxis=dict(
                    ...
                    range=[0, 4],
                    tickvals=[0, 1, 2, 3, 4],
                    ticktext=['Jamais', 'Rarement', 'Parfois', 'Souvent', 'Très souvent']
                )),
            showlegend=True,
            title="Profil détaillé par domaine de symptômes",
            height=600
        )
        ...
        if high_score_domains:
            st.markdown("**Domaines nécessitant une attention particulière :**")

            recommendations = {
                'Organisation': "📋 Utilisez des outils d'organisation (agenda, listes, applications), créez des routines structurées",
                'Attention soutenue': "🎯 Pratiquez des exercices de mindfulness, éliminez les distractions, prenez des pauses régulières",
                'Mémoire': "📝 Utilisez des rappels, notez tout, créez des associations visuelles",
                'Procrastination': "⏰ Découpez les tâches en étapes, utilisez la technique Pomodoro, fixez des échéances",
                'Hyperactivité motrice': "🏃‍♂️ Intégrez de l'exercice physique régulier, utilisez des objets anti-stress",
                'Hyperactivité mentale': "🧘 Pratiquez la méditation, apprenez des techniques de relaxation",
                'Impulsivité verbale': "🤐 Pratiquez l'écoute active, comptez jusqu'à 3 avant de parler",
                'Impulsivité comportementale': "⏸️ Développez des stratégies de pause, réfléchissez avant d'agir"
            }

            for domain in high_score_domains:
                if domain in recommendations:
                    st.write(f"• **{domain}** : {recommendations[domain]}")

        # Export des résultats
        st.subheader("💾 Sauvegarde de vos résultats")

        if st.button("📄 Générer un rapport PDF", type="secondary"):
            # Création d'un rapport simple en text
            report_text = f"""
RAPPORT DE DÉPISTAGE TDAH - ASRS-v1.1
=====================================

Date: {datetime.now().strftime('%d/%m/%Y %H:%M')}

SCORES:
- Partie A: {part_a_total}/24 ({part_a_positive}/6 critères positifs)
...
- Domaine Hyperactivité/Impulsivité: {hyperactivity_score}/36 ({(hyperactivity_score/36)*100:.1f}%)

RECOMMANDATION:
{"Consultation spécialisée recommandée" if part_a_positive >= 4 else "Surveillance et consultation si symptômes persistent" if part_a_positive >= 2 else "Pas d'indication de TDAH selon ce dépistage"}

IMPORTANT: Ce dépistage ne remplace pas un diagnostic médical professionnel.
            """
...
def main():
    ...
    <div style="text-align: center; padding: 1.5rem; background: linear-gradient(145deg, #e3f2fd, #bbdefb); border-radius: 10px; margin-bottom: 1rem;">
        <h1 style="color: #1976d2; margin-bottom: 0.5rem;">🧠 TDAH</h1>
        <p style="color: #1565c0; font-size: 1rem; margin-bottom: 0;">Dépistage & IA Avancée</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    pages = {
        ...
        "🎯 Prédiction IA": page_prediction,
        "📝 Test ASRS-v1.1": page_test_asrs
    }

    selected_page = st.sidebar.selectbox(
        "Navigation",
        list(pages.keys()),
        help="Sélectionnez la section que vous souhaitez explorer"
    )

    # Informations sur les données dans la sidebar
    st.sidebar.markdown("---")
    ...
    if df is not None and not df.empty:
        ...
        st.sidebar.info(f"📈 {len(df)} échantillons")
        st.sidebar.info(f"📋 {len(df.columns)} variables")

        if 'TDAH' in df.columns:
            tdah_count = (df['TDAH'] == 'Oui').sum()
            st.sidebar.info(f"🎯 {tdah_count} cas TDAH")
    else:
    ...
    try:
        model_data = joblib.load('best_tdah_model.pkl')
        st.sidebar.success("🤖 Modèle IA disponible")
        st.sidebar.info(f"🏆 {model_data['model_name']}")
    except FileNotFoundError:
        st.sidebar.warning("⚠️ Modèle IA non entraîné")

    # Footer de la sidebar
    st.sidebar.markdown("---")
    st.sidebar.markdown("""
    <div style="text-align: center; font-size: 0.8rem; color: #666;">
    <p>⚠️ Outil de recherche uniquement<br>
    Ne remplace pas un diagnostic médical</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    try:
        ...
        st.info("💡 Essayez de recharger la page ou sélectionnez une autre section.")
